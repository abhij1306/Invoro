...
| UCP audit is a report subsystem | `api/ucp_audit.py`, `ucp_audit/service.py`, `models/ucp_audit.py` | UCP audit creates independent audit jobs and report rows. It reuses acquisition/extraction primitives, stores findings separately, and does not mutate crawl runs, crawl surfaces, crawl records, or exports. | Compliance checks start changing crawler output or hiding extraction defects downstream |
| Product monitoring is recurring crawl orchestration | `api/monitors.py`, `api/notifications.py`, `monitor_scheduler_service.py`, `monitor_change_detection.py`, `monitor_alert_service.py`, `monitor_retention.py` | Monitors reuse normal crawl runs, track configured fields after completion, create in-app alerts for tracked field changes, retain snapshots/events by monitor retention, and never create a special extraction path | Monitor output drifts from normal crawl semantics, users miss field changes, or old history grows without bound |
| Agentic alerts are monitor-owned deltas | `api/alerts.py`, `api/public_alerts.py`, `alert_service.py`, `monitor_condition.py`, `monitor_webhook_service.py`, `monitor_change_detection.py` | Alerts are single-URL ecommerce detail monitors with second-level polling, sandboxed conditions, and webhook delivery logs. They reuse normal crawl runs and existing monitor snapshots/events; conditions only decide notification/trigger status, not whether deltas are stored. Alert naming is canonical across UI, console API, public API, and MCP; legacy `/api/v1/watches` and `/api/v1/watch` routes must remain unregistered. | A parallel alert engine forks monitor semantics, a false condition hides history, webhook code starts parsing extraction output separately, or clients see mixed watch/alert contracts |
| Product orchestration is sequencing only | `api/orchestration.py`, `orchestration_service.py`, `models/orchestration.py`, `config/orchestration_templates.py` | Projects group business context; workflows store resolved versioned template config; steps create/link normal crawl runs and monitors; result views read normal records | Business workflows fork extraction behavior, hide underlying runs, or store duplicate extracted data |
| Public API v1 is a thin launch surface | `api/public/*`, `core/public_auth.py`, `public_api/extraction_service.py`, `public_api/domain_info_service.py`, `mcp_server/*`, `api/public_alerts.py` | Public API keys authenticate `/api/v1`; rate limits are keyed by API key. Public extraction maps `surface="ecommerce"` to normal `ecommerce_detail`, runs one URL inline with HTTP-only settings, disables browser/traversal/LLM/screenshots/network capture, accepts `options.use_cache` as a documented no-op, and reads persisted `CrawlRecord.data`. Batch extraction is deferred with `WORKER_REQUIRED`; public alerts live under `/api/v1/alerts`; hosted MCP calls public REST only. | API launch starts hidden workers, forks extraction semantics, blocks the FastAPI event loop with unsafe sync work, exposes extra error-code churn, lets MCP bypass public auth/rate limits, or reintroduces stale `/api/v1/watches` routes |
...
| Sitemap mode resolves to normal batch URLs | `frontend/components/crawl/crawl-config-screen.tsx`, `crawl/batch_runtime.py`, `crawl/sitemap_resolver.py` | Category sitemap mode asks for a domain plus filter settings, resolves matching sitemap URLs at run start, then processes the resolved URLs through the normal batch loop without changing the user-selected surface | Users paste sitemap URLs manually, sitemap discovery forks crawl behavior, or listing/detail surface gets silently rewritten |
...
| UCP audit runs on demand | `api/ucp_audit.py`, `ucp_audit/service.py` | Users create a domain-scoped UCP audit job. The job persists status, options, per-page audit payloads, final JSON, and final Markdown. Background execution follows the same authenticated job pattern as Product Intelligence and Data Enrichment. | Audits become ephemeral, unowned by user, or impossible to resume/export later |
| Monitor scheduling is explicit | `monitor_scheduler_service.py`, `monitor_async_loop.py`, `tasks.py` | Active monitors run when `next_run_at <= now`, priority order is `on_demand`, `priority`, then `background`, and scheduled dispatch advances `last_run_at` / `next_run_at`. On-demand run-now dispatch does not change `next_run_at`. | Monitors silently drift schedule, on-demand clicks delay future scheduled checks, or low-priority jobs starve higher priority monitors |
| Monitor HEAD pre-check is conservative | `monitor_scheduler_service.py` | A HEAD/validator match can skip a scheduled full crawl. HEAD failure, HTTP 405 fallback failure, or missing prior state always proceeds with a full crawl. | A transient network issue causes the monitor to skip a real change |
| Monitor change detection is post-run | `monitor_change_detection.py`, `pipeline/run_complete_callbacks.py` | The pipeline only exposes one run-complete callback. Monitor-specific logic lives in the callback service and no-ops for non-monitor runs. | Pipeline body becomes monitor-specific or normal crawls start paying monitor behavior cost |
| Alert webhook dispatch follows delta detection | `monitor_change_detection.py`, `monitor_condition.py`, `monitor_webhook_service.py` | Field changes are persisted first. If no condition is set, any field change triggers; if a condition exists, webhook delivery runs only when the sandboxed condition evaluates true. Missing webhook URLs skip delivery but keep history. | Webhook delivery fires for baseline polls, false conditions erase history, or condition strings execute arbitrary code |
| Workflow orchestration preserves crawl semantics | `orchestration_service.py` | Workflow creation dispatches a normal listing run first, status polling starts the detail run from completed listing record URLs, and monitor promotion creates a normal `MonitorJob`. Intent proposal never executes. | Use-case workflows become a second crawl engine or agent auto-run path |
...
| Reset is split by data ownership | `api/dashboard.py`, `dashboard_service.py`, `frontend/components/layout/app-shell.tsx` | `Reset Workspace Data` clears crawl data, runtime files, domain memory, Product Intelligence, Data Enrichment, and UCP Audit rows; `Reset Crawl Data` clears runs/artifacts/runtime files only; `Reset Domain Memory` clears selectors, saved run profiles, cookie memory, and field feedback only | Operators lose learned memory when they intended to clear run history, or stale subsystem rows survive a supposed workspace reset |
