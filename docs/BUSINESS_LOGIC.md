...
| Public API v1 is a thin launch surface | `api/public/*`, `core/public_auth.py`, `public_api/extraction_service.py`, `public_api/domain_info_service.py`, `mcp_server/*` | Public API keys authenticate `/api/v1`; rate limits are keyed by API key. Public extraction maps `surface="ecommerce"` to normal `ecommerce_detail`, runs one URL inline with HTTP-only settings, disables browser/traversal/LLM/screenshots/network capture, accepts `options.use_cache` as a documented no-op, and reads persisted `CrawlRecord.data`. Batch/watch routes are deferred with `WORKER_REQUIRED`; hosted MCP calls public REST only. | API launch starts hidden workers, forks extraction semantics, blocks the FastAPI event loop with unsafe sync work, exposes extra error-code churn, or lets MCP bypass public auth/rate limits |
