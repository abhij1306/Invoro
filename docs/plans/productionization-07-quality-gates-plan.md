...
**Created:** 2026-08-21
**Agent:** Codex
**Status:** COMPLETE — implemented 2026-08-22; blocking CI must remain green before merge
...
- [x] Record starting commit, dirty state, existing checks, and zero-violation precondition scans in Notes.
- [x] One deterministic backend check fails on any maintained backend/repo Python file over 800 physical lines.
- [x] One deterministic backend check fails on any maintained Python callable over CC 15.
- [x] Frontend checks fail on any maintained JS/TS/TSX file over 800 physical lines.
- [x] Frontend ESLint fails on any callable/component over CC 15.
- [x] Generated/vendor exclusions are narrow, explicit, and tested.
- [x] Existing `DEFAULT_LOC_BUDGET=1000`, larger legacy ratchets, and crawl budgets of 1400/1500 are deleted or reduced to the absolute policy.
- [x] Backend CI blocks on Ruff lint, Ruff format check, mypy, LOC, complexity, dependency audit, and safe tests.
- [x] Frontend CI blocks on frozen install, dependency audit, Prettier check, ESLint including complexity, LOC/architecture, typecheck, unit tests, and build.
- [x] Frontend quality workflow runs on relevant pushes and pull requests, matching backend coverage.
- [x] Tests prove the gates fail for synthetic 801-line and CC-16 fixtures and pass at exactly 800/15 without committing oversized fixtures.
- [x] Vulture and jscpd remain diagnostic unless this PR proves a low-noise deterministic blocking policy. Do not silently turn heuristic output into a launch gate.
- [x] Targeted local workflow-equivalent verification passes and CI configuration is syntax-valid; the full safe suites remain owned by blocking CI under `ship-main`.
- [x] No product/runtime behavior changes; the sole runtime edit is a behavior-preserving split that cleared a missed CC-18 precondition violation.
...
- [x] `docs/ENGINEERING_STRATEGY.md` — make absolute physical LOC 800 and callable CC 15 mandatory hygiene gates; remove contradictory guidance.
- [x] `docs/CODEBASE_MAP.md` — no update needed because no new stable helper was created.
- [x] `docs/backend-architecture.md` / `docs/frontend-architecture.md` — record stable quality commands.
- [x] `README.md` — expose canonical local quality commands if not already covered by Plan 06.
- [x] `docs/plans/ACTIVE.md` — active pointer and final closeout.
- [x] `docs/audits/productionization-evidence-report-2026-08-21.md` — deleted after all findings closed by passing verification, per Engineering Strategy.
...
- Start state: commit `29e4c38`, branch `main`, clean worktree, synchronized with `origin/main`. Existing backend CI already ran frozen install, OSV audit, Ruff lint/format, mypy, and the safe pytest suite; frontend CI ran only on pull requests and already ran frozen install, audit, formatting, ESLint, architecture checks, typecheck, Vitest, and build.
- Start scans: 731 maintained Python files, maximum 795 physical lines, zero LOC violations; 228 maintained frontend source files, maximum 769 physical lines, zero LOC violations. The rerun found one missed Python callable at CC 18: `detail_identity_codes_from_url`. It was split in its existing identity owner without changing its public behavior; focused identity tests passed. No frontend callable exceeded CC 15.
- Final scans: 731 maintained Python files, maximum 795 physical lines, 8,325 callables, maximum CC 15; 228 maintained frontend files, maximum 768 physical lines, 2,739 callables, maximum CC 15. Both violation counts are zero.
- Boundary proofs: backend regression tests use temporary 800/801-line and CC-15/16 Python fixtures. Frontend Node tests use temporary 800/801-line and ESLint CC-15/16 fixtures and exercise every explicit artifact-directory exclusion.
- Diagnostics: Vulture 2.16 at 100% confidence reported no candidates. jscpd 4.2.4 reported 65 repository source clone leads / 897 duplicated lines / 0.57%; the four quality-owner files reported zero clones. No heuristic tool was promoted to a blocking gate.
- Local verification: frozen backend/frontend installs, OSV and pnpm audits, Ruff lint/format, mypy, backend structure regression tests, Prettier, frontend lint and boundary tests, typecheck, production build, and workflow YAML parsing passed. Full backend safe pytest and frontend Vitest suites are delegated to required CI by `ship-main`.
