...
- Final local gates: structure regression 30 passed; collection 2,459 items, 11 deselected, 2,448 selected; scoped LOC 89,669 across 278 files (down 964 from baseline); zero files over 800; zero Radon callables over 15; Ruff passed across the backend; mypy passed across 461 application source files; 315 changed-module tests passed. The final security regression adds six cases beyond that collection snapshot.
...
- After plan verification, the user explicitly expanded the ship scope to retain their concurrent frontend dependency refresh and fix all ten CodeQL alerts raised by this PR. Supported ESLint/TypeScript majors were retained, vulnerable `nanoid` paths were overridden to 3.3.18+, and hostname-spoof regressions cover the two high-severity URL findings.
