...
- Public API v1 is a lightweight FastAPI surface under `/api/v1` for Railway-style single-process deployment. API keys are dashboard-owned rows in `ApiKey`; public auth and rate limits are keyed by API key, not client IP. `POST /api/v1/extract` creates a normal single-URL crawl and runs one URL inline with HTTP-only settings, disabled LLM/browser/traversal/screenshots/network capture, and a capped timeout; public `surface="auto"` resolves to an internal concrete surface before run creation, while `ecommerce`, `content`, `article`, and `forum_thread` map to existing internal surfaces. Batch extraction remains deferred with structured `WORKER_REQUIRED`. Product alerts are active under `/api/v1/alerts` and reuse monitor-owned crawl runs, snapshots, events, and webhook delivery logs; stale `/api/v1/watches` routes are not registered. `GET /api/v1/domains/{domain}` reads existing `DomainMemory`, `DomainRunProfile`, and recent crawl rows without probing the target. `app/mcp_server/*` is a stateless FastMCP wrapper over `/api/v1`, does not import crawl services, and is installed only through the `mcp` dependency extra for dedicated MCP deployments.
...
- readiness probing receives the already-built `HtmlAnalysis` for the settled snapshot, so page settling does not parse the same HTML twice
...
- each extracted HTML artifact has one `ExtractionContext`: cleaned, original-loss fallback, and pruned working DOMs are independently cached, while rendered listing compositions keep a separate per-artifact context
- price repair and final detail cleanup accept the prepared soup, avoiding a second parse when the caller already owns the working DOM; raw-HTML entry points remain compatible
- adapter evidence is ordered once across primary HTML, rendered artifacts, and network payloads; empty and content-identical inputs are skipped globally, sufficient listings retain their early stop, and detail extraction still consumes every unique source
...
- listing admission and candidate ranking/dedupe live in `extract/listing_candidate_ranking.py`; card materialization lives in `listing_card_extraction.py`; and `listing_stages.py` owns structured/DOM/rendered stage orchestration behind the stable `listing_extractor.py` facade
- oversized extraction owners are split by existing concepts: detail identity matching (`identity/record_identity.py`), DOM option support and group assembly (`variants/dom_variant_support.py` and `variants/dom_group_pipeline.py` behind `dom_extraction.py`), long-text cleanup (`text/long_text_sanitization.py`), price reconciliation (`price/reconciliation.py`), selector values (`dom/selector_values.py`), variant group traversal (`extract/variant_choice_traversal.py`), canonical coercion (`shared/field_coerce_core.py` and `shared/field_coerce_values.py`), and Belk/Shopify product mapping (`adapters/belk_dom.py`, `adapters/shopify_products.py`)
