...
- listing candidate ranking/dedupe lives in `extract/listing_candidate_ranking.py`, admission and utility rejection live in `extract/listing_admission.py`, and `listing_stages.py` owns structured/DOM/rendered stage orchestration behind the stable `listing_extractor.py` facade
- oversized extraction owners are split by existing concepts: detail identity matching (`identity/record_identity.py`), DOM group assembly (`variants/dom_group_assembly.py`), long-text cleanup (`text/long_text_sanitization.py`), price reconciliation (`price/reconciliation.py`), selector values (`dom/selector_values.py`), variant group traversal (`extract/variant_choice_groups.py`), canonical value coercion (`shared/field_coerce_values.py`), and Belk/Shopify product mapping (`adapters/belk_dom.py`, `adapters/shopify_products.py`)
