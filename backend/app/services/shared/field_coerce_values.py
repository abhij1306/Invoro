...
from . import field_coerce as _owner
...
def _sanitize_color_scalar(value: str) -> str | None:
    if any(predicate(value) for predicate in (_SMALL_NUMERIC_RE.fullmatch, _TRACKING_PIXEL_RE.fullmatch)) or _color_value_is_opaque_code(value):
        return None
    match = re.fullmatch(r"select\s+(.+?)\s+color", value, flags=re.I)
    cleaned = clean_text(match.group(1)) if match else value
...
def _sanitize_size_scalar(value: str) -> str | None:
    ...
    if re.search(r"\b(?:please\s+)?select(?:\s+size)?\b", cleaned, flags=re.I):
        return None
    return None if cleaned.strip().lower() in _SIZE_REJECT_TOKENS_NORMALIZED else cleaned

def _sanitize_option_scalar(field_name: str, value: object) -> str | None:
    text = coerce_text(value)
    if not text or text.lstrip().startswith(("{", "[")):
        return None
    cleaned = _strip_option_suffix_noise(text) if field_name in OPTION_SCALAR_FIELDS else text
    if field_name == "color":
        cleaned = _sanitize_color_scalar(cleaned)
    elif field_name == "size":
        cleaned = _sanitize_size_scalar(cleaned)
    elif field_name == WEIGHT_FIELD and re.fullmatch(r"\d+(?:\.\d+)?", cleaned):
        return None
    return None if not cleaned or is_null_text(cleaned) else cleaned
...
def _coerce_structured_container(field_name: str, value: object) -> tuple[bool, object | None]:
    if field_name in STRUCTURED_OBJECT_FIELDS and isinstance(value, dict):
        return True, value
    if field_name not in STRUCTURED_OBJECT_LIST_FIELDS or not isinstance(value, list):
        return False, None
...
def _coerce_category_value(value: object) -> str | None:
    if isinstance(value, dict):
        ...
        value = coerce_structured_scalar(value, keys=("name", "title", "label", "value", "text", "en"))
    text = coerce_text(value)
...
def _coerce_option_value(field_name: str, value: object) -> str | None:
    ...
    if field_name == "color" and isinstance(value, list):
        readable = [item for item in value if not (isinstance(item, str) and _color_value_is_opaque_code(item))]
        scalar_input = readable or value
    scalar = coerce_structured_scalar(scalar_input, keys=(field_name, "name", "title", "label", "value", "text"))
...
def _coerce_scalar_number(field_name: str, value: object) -> tuple[bool, object | None]:
    ...
    if field_name not in _INTEGER_FIELD_NAMES:
        return False, None
    if isinstance(value, (int, float)) and not isinstance(value, bool):
        return True, int(value)
    if not isinstance(value, str):
        return False, None
    normalized = (coerce_text(value) or "").replace(",", "").strip()
...
def _coerce_mapping_field(field_name: str, value: object) -> tuple[bool, object | None]:
    ...
    coerced = coerce_rating_value(item) if field_name == "rating" else coerce_text(item)
    return True, coerced
...
def coerce_field_value(field_name: str, value: object, page_url: str) -> object | None:
    ...
    return coerce_rating_value(value) if field_name == "rating" else coerce_text(value)
...
def _coerce_product_type_clean(value: object) -> str | None:
    if isinstance(value, dict):
        value = coerce_structured_scalar(value, keys=("name", "title", "label", "value", "text", "type"))
    text = coerce_text(value)
    if not text:
        return None
