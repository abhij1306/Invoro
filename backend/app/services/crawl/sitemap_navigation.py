...
def build_nav_tree(
    ...
    labels = {url_key(url): label for url, label in (labels_by_url or {}).items() if label}
...
def _append_nav_path(*, parsed, segments, roots, child_maps, labels, url_by_key) -> None:
    parent_children = roots
    current_path: list[str] = []
    for segment in segments:
        current_path.append(segment)
        prefix_url = urlunsplit((parsed.scheme, parsed.netloc, "/" + "/".join(current_path), "", ""))
...
def classify_homepage_candidate(*, candidate_url: str, keyword: str, anchor: Tag) -> tuple[str, int]:
    resolution = resolve_auto_surface(url=candidate_url)
    path = urlsplit(candidate_url).path.lower().rstrip("/")
    depth = path_depth(path)
    anchor_words = len([word for word in " ".join(anchor.stripped_strings).split() if word])
    ...
    if resolution.surface.endswith("_detail"):
        return "detail", 220 + int(resolution.confidence * 100) + (25 if keyword_hit else 0)
...
def _keyword_hit(keyword: str, *, candidate_url: str, anchor: Tag) -> bool:
    ...
    return bool(keyword) and (keyword in candidate_url.lower() or keyword in anchor_text)
...
def looks_like_listing_link(path: str, *, depth: int, anchor_words: int) -> bool:
    if depth == 0 or depth > 2:
        return False
    if anchor_words == 0 or anchor_words > SITEMAP_HOMEPAGE_FALLBACK_MAX_LINK_TEXT_WORDS:
        return False
...
def looks_like_detail_link(slug: str, *, depth: int, anchor_words: int) -> bool:
    if depth < 2 or anchor_words == 0 or anchor_words > 12:
        return False
    return any(char.isdigit() for char in slug) or slug.count("-") >= 2 or slug.count("_") >= 2
...
def looks_like_category_url(url: str) -> bool:
    ...
    if any(token in segment_text for token in SITEMAP_CATEGORY_ANCHOR_TEXT_EXCLUDED_TOKENS):
        return False
    category_segments = {token for token in SITEMAP_CATEGORY_ANCHOR_TEXT_TOKENS if token and " " not in token}
...
def _category_segments_are_plausible(segments: list[str]) -> bool:
    return (
        ...
        and not any(looks_like_locale_segment(segment) or segment.isdigit() for segment in segments)
    )
...
def looks_like_locale_path(path: str) -> bool:
    ...
    return bool(parts) and len(parts) <= 2 and all(looks_like_locale_segment(part.replace("_", "-")) for part in parts)
...
def path_depth(path: str) -> int:
    return len([part for part in path.split("/") if part and not looks_like_locale_segment(part)])
...
def looks_like_locale_segment(value: str) -> bool:
    ...
    if len(cleaned) == 2 and cleaned.isalpha():
        return True
    return len(cleaned) == 5 and cleaned[2] == "-" and cleaned[:2].isalpha() and cleaned[3:].isalpha()
...
def label_from_path_segment(segment: str) -> str:
    ...
    return " ".join(word.capitalize() for word in cleaned.split()) if cleaned else segment
