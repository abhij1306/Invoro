...
from . import dom_extraction as _owner
...
def _collect_select_candidate_groups(
    ...
    *,
    ...
    for select in iter_variant_select_groups(soup):
        raw_option_values = [
            clean_text(option.get_text(" ", strip=True)) for option in select.find_all("option") if clean_text(option.get_text(" ", strip=True))
        ]
        ...
        if not cleaned_name:
            continue
        component_style = _component_size_style_from_group_name(cleaned_name) or _component_size_style_from_group_name(next(iter(raw_option_values), ""))
        if component_style:
            cleaned_name = "size"
        ...
        deduped_values = list(dict.fromkeys(str(entry["value"]) for entry in option_entries if text_or_none(entry.get("value"))))
...
def _select_option_entry(
    ...
    *,
    ...
    if not cleaned_value or variant_option_value_is_noise(cleaned_value) or (raw_value_attr is not None and raw_value_attr.lower() in {"select", "choose"}):
        return None
...
def _collect_choice_candidate_groups(
    ...
    *,
    ...
    for container in iter_variant_choice_groups(soup):
        ...
        values = [str(entry["value"]) for entry in option_entries if text_or_none(entry.get("value"))]
...
def _merge_dom_option_groups(
    ...
    for group in groups:
        ...
        if len(name) > len(str(merged.get("name") or "")):
            merged["name"] = name
        merged["values"] = list(dict.fromkeys([*_object_list(merged.get("values")), *values]))
...
def _assemble_dom_variant_rows(
    ...
    *,
    ...
    for combo in product(*axis_value_lists):
        option_values = {axis_name: value for axis_name, value in zip(axis_names, combo, strict=False) if clean_text(value)}
        if option_values:
            rows.append(
...
def backfill_variants_from_dom_if_missing(
    ...
    *,
    ...
    dom_variant_rows = [row for row in _object_list(dom_variants.get("variants")) if isinstance(row, dict)]
    if not dom_variant_rows:
        return
    if (
        record_has_rich_existing_variants(record) or existing_variant_cluster_has_transport_signal(existing_variants)
...
def _merge_dom_variant_rows(existing_variants: list[dict[str, Any]], dom_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    expanded = _expand_existing_variants_with_dom_axes(existing_variants, dom_rows)
    if expanded:
        return expanded
    existing_by_key: dict[str, dict[str, Any]] = {}
    for row in existing_variants:
        row_key = text_or_none(row.get("variant_id")) or text_or_none(row.get("url"))
        if row_key:
            existing_by_key.setdefault(row_key, row)
    merged_rows: list[dict[str, Any]] = []
    for dom_row in dom_rows:
        dom_key = text_or_none(dom_row.get("variant_id")) or text_or_none(dom_row.get("url"))
