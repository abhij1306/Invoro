...
def _node_has_attr(node: Any, attr_name: str) -> bool:
    return bool(hasattr(node, "has_attr") and node.has_attr(attr_name))


def _option_node_is_selected(node: Any, label_node: Any | None) -> bool:
    for candidate in (
        node,
        label_node,
        ...
        if _node_has_attr(candidate, "checked") or _node_has_attr(
            candidate, "selected"
        ):
            return True
...
def _option_node_is_disabled(node: Any, label_node: Any | None) -> bool:
    for candidate in (
        node,
        label_node,
        getattr(node, "parent", None),
        ...
        if candidate is None or not hasattr(candidate, "get"):
            continue
        if node_attr_is_truthy(candidate, "data-disabled"):
            return True
        for attr_name in VARIANT_OPTION_DISABLED_ATTRIBUTE_NAMES:
            if attr_name != "aria-disabled" and _node_has_attr(candidate, attr_name):
                return True
            if node_attr_is_truthy(candidate, attr_name):
                return True
        if node_state_matches(candidate, *VARIANT_OPTION_DISABLED_CLASS_TOKENS):
            return True
    return False


def _option_flag_availability(node: Any, label_node: Any | None) -> str | None:
    candidates = (
        node,
        label_node,
        getattr(node, "parent", None),
    ...
    for candidate in candidates:
        if candidate is None or not hasattr(candidate, "get"):
            continue
        if node_attr_is_truthy(
            candidate, *VARIANT_OPTION_OUT_OF_STOCK_FLAG_ATTRIBUTE_NAMES
        ):
            return "out_of_stock"
    for candidate in candidates:
        if candidate is None or not hasattr(candidate, "get"):
            continue
        for attr_name in VARIANT_OPTION_AVAILABLE_FLAG_ATTRIBUTE_NAMES:
            raw = candidate.get(attr_name)
            if raw in (None, "", [], {}):
                continue
            normalized = str(raw).strip().lower()
            if normalized in VARIANT_OPTION_FLAG_FALSE_VALUES:
                return "out_of_stock"
            return "in_stock"
    return None
