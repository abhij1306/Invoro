...
def _reconcile_detail_price_currency(
    ...
    evidence.html_currency_conflicts_with_host = _html_currency_conflicts_with_strong_host_hint(
...
def _apply_detail_record_price(
    ...
    *,
    ...
    if (
        ...
        and not (record_field_sources(record, "price") & DETAIL_AUTHORITATIVE_PRICE_SOURCE_SET)
    ):
...
def _backfill_detail_variant_prices(
    ...
    *,
    ...
    if not isinstance(variants, list):
        return
    authoritative_variants = bool(record_field_sources(record, "variants") & DETAIL_AUTHORITATIVE_PRICE_SOURCE_SET)
    for variant in variants:
        if not isinstance(variant, dict):
            continue
        if source == FIELD_SOURCE_JSON_LD and jsonld_currency and _detail_price_is_visible_outlier(variant.get("price"), price) and not authoritative_variants:
            variant["price"] = price
            variant["currency"] = jsonld_currency
            continue
        _backfill_variant_price(variant, price=price, currency=currency)

from . import reconciliation as _split_owner
globals().update({
    name: value
