    ...
    *,
    ...
    for variant in variants:
        if not isinstance(variant, dict):
            continue
        if _variant_row_looks_like_foreign_product(
            record, variant, color_extractor=color_extractor
        ):
            continue
...
def prune_axisless_rows_when_axisful_rows_exist(record: dict[str, Any]) -> None:
    ...
    pruned = [
        ...
        and not _drop_axisless_variant_row(
            variant, semantic_variant_count=len(semantic_keys)
        )
    ]
...
def _variant_row_looks_like_foreign_product(
    ...
    *,
    ...
    if len(color_tokens) < max(
        3, _VARIANT_OPTION_LABEL_MAX_WORDS - 1
    ) or not color_extractor(color_value):
        return False
...
def _variant_row_axis_map(variant: dict[str, Any]) -> dict[str, str]:
    return {
        axis: clean_text(variant.get(axis))
        for axis in _PUBLIC_VARIANT_AXIS_FIELDS
        if clean_text(variant.get(axis))
    }
...
def _drop_axisless_variant_row(
    variant: dict[str, Any], *, semantic_variant_count: int
) -> bool:
...
def _variant_sku_is_size_specific_child(
    *, parent_sku: str, child_sku: str, size: str
) -> bool:
...
def _variant_matches_parent_price_signal(
    ...
    *,
    ...
    return text_or_none(variant.get("price")) in (None, parent_price) and text_or_none(
        variant.get("currency")
    ) in (None, parent_currency)


def _replace_or_drop_variants(
    record: dict[str, Any], variants: list[dict[str, Any]]
) -> None:
    if variants:
