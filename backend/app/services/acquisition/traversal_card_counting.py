...
async def _selector_card_counts(
    page: Page, *, selectors: list[str], surface: str
) -> dict[object, object] | int:
    try:
...
def _resolve_selector_card_count(
    selector_counts: dict[object, object], *, allow_heuristic: bool
) -> tuple[int, bool]:
    strong_count = 0
    weak_count = 0
    ...
    sparse_strong_count = strong_count < max(
        3, int(crawler_runtime_settings.listing_min_items) + 1
    )
    if strong_count > 0:
