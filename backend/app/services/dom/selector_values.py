...
from app.services.dom.selector_scope import (
    best_text_scope,
    clone_visible_only,
    field_uses_scoped_text,
    is_other_detail_link,
    node_is_hidden_or_auxiliary,
    node_within_scope,
)
...
def extract_selector_values(
    ...
    values: list[object] = []
    scoped_text_root = (
        best_text_scope(root) if field_uses_scoped_text(field_name) else None
    )
    for node in safe_select(root, selector)[:_max_selector_matches]:
        if field_uses_scoped_text(field_name):
            if node_is_hidden_or_auxiliary(node):
                continue
            if scoped_text_root is not None and not node_within_scope(
...
def extract_page_images(
    ...
    *,
    ...
    return extract_page_images_impl(
        ...
        surface=surface,
        other_detail_link_checker=is_other_detail_link,
    )
