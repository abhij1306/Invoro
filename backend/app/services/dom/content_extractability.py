...
def requested_content_extractability_impl(
    ...
    *,
    ...
    requested = _requested_field_keys(requested_fields)
    alias_lookup = surface_alias_lookup(surface, requested_fields)
    fields = _probe_field_scope(surface, requested_fields, probe_fields)
    field_scope = set(fields)
    section_fields = _section_field_matches(
        root, alias_lookup, field_scope, extract_heading_sections
    )
    dom_pattern_fields = _dom_pattern_field_matches(
        root, fields, safe_select, max_selector_matches
    )
    selector_backed_fields = _selector_backed_field_matches(selector_rules, field_scope)
    ...
    return {
        ...
        "selector_backed_fields": sorted(selector_backed_fields),
    }


def _requested_field_keys(requested_fields: list[str] | None) -> set[str]:
    requested = {
        normalized
...
def _probe_field_scope(
    surface: str,
    requested_fields: list[str] | None,
    probe_fields: list[str] | tuple[str, ...] | set[str] | None,
) -> list[str]:
    return (
        [
            field_name
...
def _dom_pattern_field_matches(
    root: BeautifulSoup | Tag,
    fields: list[str],
    safe_select: Callable[[BeautifulSoup | Tag, str], list[Tag]],
    max_selector_matches: int,
...
def _selector_backed_field_matches(
