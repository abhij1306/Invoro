...
async def crawl_catalog(domain: str, *, sample_size: int = 5) -> CatalogCrawlResult:
    ...
    if listing is not None:
        ...
        if not detail_urls and category_urls:
            detail_urls = await _discover_detail_urls_from_categories(
                category_urls,
    ...
    for detail_url in detail_urls:
        ...
        if records:
            ...
            if _first_product_jsonld(structured_record.get("_jsonld") or []):
                result.product_records.append(structured_record)
            else:
                result.crawl_errors.append(
                    f"{final_url}: rejected non-product sample after detail extraction returned no record"
                )
...
def _listing_candidate_urls(
    ...
    for record in records:
        ...
        if not _same_host(page_url, absolute):
            continue
        if detail_url_looks_like_product(absolute):
            detail_urls.append(absolute)
        else:
            category_urls.append(absolute)
    return (
        list(dict.fromkeys(detail_urls))[:sample_size],
        list(dict.fromkeys(category_urls))[: config.AID_DISCOVERY_CATEGORY_MAX_PAGES],
    )


async def _discover_detail_urls_from_categories(
    category_urls: list[str],
    *,
    sample_size: int,
    result: CatalogCrawlResult,
    ...
    detail_urls: list[str] = []
    for category_url in category_urls[: config.AID_DISCOVERY_CATEGORY_MAX_PAGES]:
        if len(detail_urls) >= sample_size:
            break
        page = await _fetch(category_url, surface="ecommerce_listing", result=result)
        if page is None:
            continue
        result.pages_crawled += 1
        final_url = _final_url(page, category_url)
        discovered, _ = _listing_candidate_urls(
            page,
            final_url,
            sample_size - len(detail_urls),
            result,
        )
        detail_urls.extend(discovered)
    return list(dict.fromkeys(detail_urls))[:sample_size]
...
def _clean_url(url: str) -> str:
    ...
    embedded_query = ""
    if "?" in decoded_path:
        decoded_path, embedded_query = decoded_path.split("?", 1)
    query = "&".join(part for part in (embedded_query, parsed.query) if part)
    cleaned_query = urlencode(
        [
            (key, value)
            for key, value in parse_qsl(query, keep_blank_values=True)
            if key.lower() not in config.AID_TRACKING_QUERY_KEYS
        ],
        doseq=True,
    )
    return urlunparse(
        (parsed.scheme, parsed.netloc, decoded_path or "/", "", cleaned_query, "")
    )


def _has_encoded_tracking_query(url: str) -> bool:
    lowered = unquote(str(url or "")).lower()
    return any(f"{key.lower()}=" in lowered for key in config.AID_TRACKING_QUERY_KEYS)
