    ...
    *,
    ...
    for field, values in (("price", prices), ("original_price", original_prices)):
        value = normalize_price(
            _array_value(values, index), interpret_integral_as_cents=False
        )
        if value is not None:
            row[field] = value
    _apply_sku_inventory(
        row, _array_value(out_of_stock, index), _array_value(inventories, index)
    )
...
def _apply_sku_inventory(
    row: dict[str, object], out_of_stock: object, inventory: object
) -> None:
