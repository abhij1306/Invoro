...
export default function SelectorsPage() {
  ...
  async function saveAcceptedRows() {
    ...
    if (failedFields.length) {
      setLoadError(
        `Unable to save ${failedFields.join(', ')}. Saved rows stay marked as saved; failed rows remain accepted for retry.`,
      );
    }
  }
        ...
        >
          <div className="bg-panel shadow-card overflow-hidden rounded-none p-0 backdrop-blur-md">
            {previewUrl ? (
              ...
              <div className="text-muted grid h-[760px] place-items-center text-sm leading-[var(--leading-relaxed)]">
                No page loaded.
              </div>
            )}
          </div>
